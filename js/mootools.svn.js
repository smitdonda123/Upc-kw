...
$native.generic = function (prop) {
  return function (bind) {
    return this.prototype[prop].apply(
      bind,
  ...
  setOptions: function () {
    ...
    if (this.addEvent) {
      for (var option in this.options) {
        if (
          $type(this.options[option] == "function") &&
          /^on[A-Z]/.test(option)
        )
...
String.extend({
  ...
  test: function (regex, params) {
    return ($type(regex) == "string" ? new RegExp(regex, params) : regex).test(
      this
    );
  },
...
Element.extend = function (properties) {
  for (var property in properties) {
    ...
    var elementsProperty = Array.prototype[property]
      ? property + "Elements"
      : property;
...
Element.fixStyle = function (property, result, element) {
  ...
  if (["height", "width"].contains(property)) {
    ...
    values.each(function (value) {
      size +=
        element.getStyle("border-" + value + "-width").toInt() +
...
Element.Methods.Events = {
  ...
  removeEvent: function (type, fn) {
    ...
    return Element.NativeEvents.contains(type)
      ? this.removeListener(type, value)
      : this;
  },
...
$$.shared = {
  ...
  normal: {
    getParam: function (items, context, param, i) {
      ...
      if (param[4])
        items = Elements.filterByAttribute(
          items,
          param[4],
          param[5],
          param[6],
  ...
  add: function (fn) {
    ...
    if (document.readyState && window.webkit) {
      ...
      if (!$("ie_ready")) {
        ...
        document.write(
          '<script id="ie_ready" defer src="' + src + '"></script>'
        );
...
Fx.Scroll = Fx.Base.extend({
  ...
  scrollTo: function (x, y) {
    ...
    for (var z in el.size) {
      ...
      if ($chk(values[z]))
        values[z] =
          $type(values[z]) == "number" ? values[z].limit(0, max) : max;
...
Fx.Transitions.compat = function (transition) {
  ["In", "Out", "InOut"].each(function (easeType) {
    Fx.Transitions[transition.toLowerCase() + easeType] =
...
Fx.Transitions.extend({
  ...
  Elastic: function (p, x) {
    return (
      Math.pow(2, 10 * --p) * Math.cos((20 * p * Math.PI * (x[0] || 1)) / 3)
    );
  },
});
...
Drag.Base = new Class({
  ...
  start: function (event) {
    ...
    for (var z in this.options.modifiers) {
      ...
      this.value.now[z] = this.element
        .getStyle(this.options.modifiers[z])
        .toInt();
      ...
      if (limit && limit[z]) {
        for (var i = 0; i < 2; i++) {
          if ($chk(limit[z][i]))
            this.limit[z][i] =
              $type(limit[z][i]) == "function" ? limit[z][i]() : limit[z][i];
        }
      }
    }
...
Element.extend({
  ...
  makeResizable: function (options) {
    return new Drag.Base(
      this,
      $merge({ modifiers: { x: "width", y: "height" } }, options)
    );
  },
});
...
Drag.Move = Drag.Base.extend({
  ...
  drag: function (event) {
    ...
    if (this.overed != overed) {
      ...
      this.overed = overed
        ? overed.fireEvent("over", [this.element, this])
        : null;
    }
    return this;
  },
  ...
  checkAgainst: function (el) {
    ...
    return (
      now.x > el.left && now.x < el.right && now.y < el.bottom && now.y > el.top
    );
  },
...
Object.toQueryString = function (source) {
  ...
  for (var property in source)
    queryString.push(
      encodeURIComponent(property) + "=" + encodeURIComponent(source[property])
    );
  return queryString.join("&");
};
...
Element.extend({
  ...
  send: function (options) {
    return new Ajax(
      ...
      $merge({ data: this.toQueryString() }, options, { method: "post" })
    ).request();
  },
});
    ...
    switch ($type(obj)) {
      ...
      case "object":
        ...
        for (var property in obj)
          string.push(
            Json.toString(property) + ":" + Json.toString(obj[property])
          );
        return "{" + string.join(",") + "}";
      case "number":
...
var Color = new Class({
  ...
  mix: function () {
    ...
    var alpha =
      $type(colors[colors.length - 1]) == "number" ? colors.pop() : 50;
    var rgb = this.copy();
...
var Scroller = new Class({
  ...
  initialize: function (element, options) {
    this.setOptions(options);
    this.element = $(element);
    this.mousemover = [window, document].contains(element)
      ? $(document.body)
      : this.element;
  },
  ...
  scroll: function () {
    ...
    for (var z in this.page) {
      if (this.page[z] < this.options.area + pos[z] && el.scroll[z] != 0)
        change[z] =
          (this.page[z] - this.options.area - pos[z]) * this.options.velocity;
      else if (
        this.page[z] + this.options.area > el.size[z] + pos[z] &&
        el.scroll[z] + el.size[z] != el.scrollSize[z]
      )
        change[z] =
          (this.page[z] - el.size[z] + this.options.area - pos[z]) *
          this.options.velocity;
    }
    if (change.y || change.x)
      this.fireEvent("onChange", [
        el.scroll.x + change.x,
...
var Slider = new Class({
  ...
  initialize: function (el, knob, options) {
    ...
    this.max =
      this.element[offset] - this.knob[offset] + this.options.offset * 2;
  ...
  toStep: function (position) {
    return Math.round(
      ((position + this.options.offset) / this.max) * this.options.steps
    );
  },
