  ...
  var LofFlashContent = new Class({
    initialize: function (eMain, eNavigator, eNavOuter, options) {
            ...
            item.setStyles(
              eval(
                '({"' +
                  ...
                  '": index * this.maxSize,"' +
                  this.styleMode[1] +
                  '":Math.abs(this.maxSize),"display" : "block"})'
              )
            );
      ...
      if ($defined(eNavigator) && $defined(eNavOuter)) {
        ...
        eNavOuter.setStyle(
          "height",
          this.setting.navItemsDisplay * this.setting.navItemHeight
        );
    ...
    navivationAnimate: function (currentIndex) {
      if (
        currentIndex <= this.setting.startItem ||
        currentIndex - this.setting.startItem >=
          this.setting.navItemsDisplay - 1
      ) {
        this.setting.startItem =
          currentIndex - this.setting.navItemsDisplay + 2;
        if (this.setting.startItem < 0) this.setting.startItem = 0;
        if (
          this.setting.startItem >
          this.slides.length - this.setting.navItemsDisplay
        ) {
          this.setting.startItem =
            this.slides.length - this.setting.navItemsDisplay;
        }
      ...
      this.navigatorFx
        .stop()
        .start(-this.setting.startItem * this.setting.navItemHeight);
    },
      ...
      if ($defined(objects) && this.slides.length > 1) {
        for (var action in objects) {
          if ($defined(this[action.toString()]) && $defined(objects[action])) {
            objects[action].addEvent(
              eventHandler,
              this[action.toString()].bind(this, [true])
            );
          }
        }
      }
    ...
    onProcessing: function (manual, start, end) {
      ...
      this.nextNo =
        this.currentNo +
        (this.currentNo < this.slides.length - 1 ? 1 : 1 - this.slides.length);
      return this;
    },
    ...
    jumping: function (no, manual) {
      ...
      this.onProcessing(null, manual, 0, this.maxSize)
        ...
        .fxStart(
          this.currentNo,
          this.getObjectDirection(this.minSize, -this.maxSize)
        )
        .finishFx(manual);
      this.currentNo = no;
    },
    next: function (manual, item) {
      ...
      this.onProcessing(item, manual, 0, this.maxSize)
        .fxStart(
          this.currentNo,
          this.getObjectDirection(this.maxSize, this.minSize)
        )
        .fxStart(
          this.previousNo,
          this.getObjectDirection(this.minSize, -this.maxSize)
